...
def fuzzylabels(ts, alts, spds, twindow=60):
    ...
    if len(set([len(ts), len(alts), len(spds)])) > 1:
...
#                {spd_level_lo:.3f}')
        ...
        rule_climb = min(alt_level_lo, spd_level_md)
        state_activate_climb = np.fmin(rule_climb, state_climb)

        rule_descent = min(alt_level_lo, spd_level_md)
        state_activate_descent = np.fmin(rule_descent, state_descent)

        rule_cruise = min(alt_level_hi,  spd_level_hi)
        state_activate_cruise = np.fmin(rule_cruise, state_cruise)

        rule_level = min(alt_level_lo, spd_level_md)
