...
# if NCorr == 1: # Single aisle
#         # Inner fuselage
#         teta=(0:np.pi/20:2*np.pi)
#         # eixox=2*a_mini
#         # eixoy=eixox*height_witdh_ratio
#         x=(eixox/2)*np.cos(teta)
#         y=(eixoy/2)*np.sin(teta)
#         plot(x,y)
#         hold on
#         # Outer fuselage
#         eixoxe=eixox + 2*fuselage_thickness
#         eixoye=eixoy + 2*fuselage_thickness
#         x=(eixoxe/2)*np.cos(teta)
#         y=(eixoye/2)*np.sin(teta)
#         plot(x,y)
#         hold on
#         # *** Floor ***
#         # Parte superior
...
#         yp(2)=yp(1)
#         tetapiso=np.asin(2*FUSELAGE_Dz_floor/eixoy)
#         xp(1)=(eixox/2)*np.cos(tetapiso)
#         xp(2)=(eixox/2)*np.cos(tetapiso+pi)
#         plot(xp,yp)
#         hold on
#         # parte inferior 
...
#                 ybotarm = FUSELAGE_Dz_floor -  SEAT_h_armrest_bottom
#         # end
#         tetabot = np.asin(2*ybotarm/eixoy)
#         xbotarm = (eixox/2)*np.cos(-tetabot)
...
#         FUSELAGE_dseat_seat_rail_esq = 0.20 # distancia da extremidade do assento � primeira fileira de trilhos
...
#         if FUSELAGE_dir > 1 and FUSELAGE_esq > 1:
        
#         tetaover = np.asin(2*h1/eixoy)
#         xee    = (eixox/2)*np.cos(tetaover+pi)
#         xov[0] = (eixox/2)*np.cos(tetaover)
#         yov[0] = h1
#         xov[1] = xov[0] - SEAT_w_cushion_YC/2
#         yov[1] = h1
#         xov[2] = orix + FUSELAGE_w_aisle_YC + SEAT_w_cushion_YC/2
#         yov[2] = h1 +0.005
#         xov[3] = xov[2] + 0.05
#         yov[3] = FUSELAGE_h_aisle - FUSELAGE_Dz_floor - 0.05
#         xov[4] = orixn
#         yov[4] = FUSELAGE_h_aisle - FUSELAGE_Dz_floor

#         fator = 3
#         if FUSELAGE_esq > 2:
#                 fator = 5

#         xov[5] = xee + fator*SEAT_w_cushion_YC/2 - 0.05
#         yov[5] = yov[3]
#         xov[6] = xee + fator*SEAT_w_cushion_YC/2
#         yov[6] = h1 + 0.005
#         xov[7] = xee + SEAT_w_cushion_YC/2
#         yov[7] = h1
#         xov[8] = xee
#         yov[8] = h1
#         # plot(xov,yov,'c')
