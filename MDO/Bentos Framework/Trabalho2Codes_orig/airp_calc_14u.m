...
function [DOCcalc, Flag_constraints,...
    fuselage, wing, ht,vt,pylon,engine,wlet]=airp_calc_14u(x)
%
datasimulacao=date;
jahr='2019V15b';
fprintf('\n %s \n',datasimulacao)
...
IPLOT=0; % =0 no plot; ~=0 will be plot the top view of the airplane 
% ***********************  Airplane VARIABLES *****************************
% Fuselage 
widthreiratio       = x(8); % Fuselage height-to-width ratio
NPax                = 100; % single class
NAisle              = 1;   
NSeat               = 5;   % Seating abreast
SeatPitch           = 0.8128; % Distance between seats [m]
% Wing --------------------------------------------------------------------
wS                  = x(1);  % Wing reference area [m2]
wAR                 = x(2);  % Wing aspect ratio
wTR                 = x(3); % Wing taper ratio
wSweep14            = x(4); % Quarter-chord wing sweepback angle
wTwist              = x(5);
Kink_semispan       = x(6);
longtras            = 0.76; % Fraction of chord where the rear spar is located
% Engine data  ------------------------------------------------------------
...
grid on
print('-f19','Figures/MTOW_iteration.png','-dpng','-r350')
close(figure(19))
% ******************* Check requirements **********************************
