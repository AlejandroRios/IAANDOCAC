...
########################################################################################
fig, ax = plt.subplots()
m = Basemap(resolution='i', projection='merc', llcrnrlat=38, urcrnrlat=52, llcrnrlon=-5, urcrnrlon=14)
...
df = pd.read_csv('Data4Clustering3.csv', header=0, delimiter=',')
df_head = df.head()
...
def plot_cluster(traj_lst, cluster_lst):
    ...
    plt.legend((p0[0],p1[0],p2[0]),('Outliers','Cluster 1','Cluster 2'))
