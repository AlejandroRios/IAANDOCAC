...
mms_alt = preprocessing.MinMaxScaler(feature_range=(0, 1))
...
def plot_cluster(traj_lst, cluster_lst):
    ...
    for traj, cluster in zip(traj_lst, cluster_lst):
        ...
        if cluster == -1:
            x_aux1 = traj[:,0]
            x_aux1 = x_aux1.reshape(-1, 1)
            x_aux1 = mms_lon.inverse_transform(x_aux1)[:,0]

            y_aux1 = traj[:,1]
            y_aux1 = y_aux1.reshape(-1, 1)
            y_aux1 = mms_lat.inverse_transform(y_aux1)[:,0]

            x1, y1 = m(x_aux1, y_aux1)
            p0 = m.plot(*(x1, y1), c='k', linestyle='dashed',linewidth=0.1,alpha=0.5)
        elif cluster == 0:
            x_aux2 = traj[:,0]
            x_aux2 = x_aux2.reshape(-1, 1)
            x_aux2 = mms_lon.inverse_transform(x_aux2)[:,0]

            y_aux2 = traj[:,1]
            y_aux2 = y_aux2.reshape(-1, 1)
            y_aux2 = mms_lat.inverse_transform(y_aux2)[:,0]

            x2, y2 = m(x_aux2, y_aux2)
            p1 = m.plot(*(x2, y2), c=color_lst[cluster % len(color_lst)],linewidth=0.5,alpha=0.5)
        elif cluster == 1:
            x_aux3 = traj[:,0]
            x_aux3 = x_aux3.reshape(-1, 1)
            x_aux3 = mms_lon.inverse_transform(x_aux3)[:,0]

            y_aux3 = traj[:,1]
            y_aux3 = y_aux3.reshape(-1, 1)
            y_aux3 = mms_lat.inverse_transform(y_aux3)[:,0]

            x3, y3 = m(x_aux3, y_aux3)
            p2 = m.plot(*(x3, y3), c=color_lst[cluster % len(color_lst)],linewidth=0.5,alpha=0.5)
        elif cluster == 2:
            x_aux4 = traj[:,0]
            x_aux4 = x_aux4.reshape(-1, 1)
            x_aux4 = mms_lon.inverse_transform(x_aux4)[:,0]

            y_aux4 = traj[:,1]
            y_aux4 = y_aux4.reshape(-1, 1)
            y_aux4 = mms_lat.inverse_transform(y_aux4)[:,0]

            x4, y4 = m(x_aux4, y_aux4)
