...
#################################################################################################################
mms = preprocessing.MinMaxScaler(feature_range=(0, 1))
lat_testee = mms.fit_transform(lat_teste_ff)
mms = preprocessing.MinMaxScaler(feature_range=(0, 1))
lon_testee = mms.fit_transform(lon_teste_ff)
...
dbscan = DBSCAN(eps=0.006, min_samples = 8)
...
fig = plt.plot(lon_testee,lat_testee,'v',)
...
lon_clus = pd.Series(coord_clus[:, 0]) 
lat_clus = pd.Series(coord_clus[:, 1]) 
...
teste2x = np.concatenate([lon_testee,lat_testee],axis=1)

previsoes = modelo.predict(teste2x)
print(previsoes)
